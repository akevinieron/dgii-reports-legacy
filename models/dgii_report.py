...
class DgiiReport(models.Model):
    ...
    def _purchase_report_totals(self):
        ...
        for rec in self:
            ...
            for purchase in rec.purchase_report:
                if purchase.NUMERO_COMPROBANTE_FISCAL[1:3] == "04":
    ...
    def get_payment_date_and_retention_data(self, invoice_id):
        ...
        if invoice_id.journal_id.purchase_type == 'informal':
            if not move_id:
                retentions = self.env["account.move.line"].search([('invoice_id', '=', invoice_id.id),
                                                                   ('payment_id', '!=', False),
                                                                   ('tax_line_id', '!=', False)])
                if retentions:
    ...
    def create_sales_lines(self, data):
        data_text = b','.join(
            self.env.cr.mogrify('(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', row) for row in data).decode()
    ...
    def create_purchase_lines(self, data):
        data_text = b','.join(
            self.env.cr.mogrify('(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', row) for row in data).decode()
    ...
    def post_error_list(self, error_list):
        ...
        if error_list:
            message = "<ul>"
            for ncf, errors in error_list.items():
    ...
    def generate_report(self):
        ...
        journal_ids = self.env["account.journal"].search(['|', ('ncf_control', '=', True),
    ...
    def generate_txt_files(self):
        ...
        for sale_line in self.sale_report:
            ...
            ln += sale_line.NUMERO_COMPROBANTE_FISCAL.rjust(11)
            ln += sale_line.NUMERO_COMPROBANTE_MODIFICADO or "".rjust(11)
        ...
        for line in self.purchase_report:
            ...
            ln += line.NUMERO_COMPROBANTE_FISCAL and line.NUMERO_COMPROBANTE_FISCAL.rjust(11) or "".rjust(11)
            ln += line.NUMERO_COMPROBANTE_MODIFICADO or "".rjust(11)
...
class DgiiReportPurchaseLine(models.Model):
    ...
    NUMERO_COMPROBANTE_FISCAL = fields.Char("NCF", size=11)
    NUMERO_COMPROBANTE_MODIFICADO = fields.Char("Afecta", size=11)
